...
#             for row in reader:
#                 test_case_name = row["Test case name"].strip()
#                 sw_v1_result = row.get("SW v1", "").strip().upper()
#                 previous_results[test_case_name] = sw_v1_result
#     return previous_results
...
#             writer.writerow([
#                 "Job number from jenkins", "Date time", "Test case name",
#                 f"SW v1 ({sw_version_v1})", f"SW v2 ({sw_version_v2})", "Error Message (if any)"
#             ])
#         writer.writerow([
#             job_number, timestamp, test_name,
#             sw_v1_result, sw_v2_result, error_message
...
#     for result in test_results:
#         sw_v2_result = result["sw_v2_result"]
#         sw_v1_result = result["sw_v1_result"]
...
#     print(f"{Fore.GREEN}Passed: {summary['passed']}{Style.RESET_ALL}")
#     print(f"{Fore.RED}Failed: {summary['failed']}{Style.RESET_ALL}")
#     print(f"{Fore.YELLOW}No Previous Run (NA): {summary['na']}{Style.RESET_ALL}")
#     print(f"{Fore.MAGENTA}Mismatches between SW v1 and SW v2: {summary['mismatch']}{Style.RESET_ALL}")
#     print(f"{Fore.BLUE}Selected: {selected}, Deselected: {deselected}{Style.RESET_ALL}")
#     print(border)
#     if summary['total'] > 0:
#         print(f"Pass Rate: {(summary['passed'] / summary['total']) * 100:.2f}%")
...
#                     break
#     except:
#         sw_version_v2 = os.getenv("SOFTWARE_VERSION", "3.220.1.30")

#     # Define previous version (can be read from previous report header if needed)
#     sw_version_v1 = "3.220.1.29"
...
#         test_name = test.get("nodeid", "unknown_test").strip()
#         sw_v2_result = test.get("outcome", "UNKNOWN").upper()
#         sw_v1_result = previous_results.get(test_name, "NA")  # Default to NA
#         error_message = ""

#         if sw_v2_result == "FAILED":
#             error_message = test.get("call", {}).get("longrepr", "No details")
#         elif sw_v1_result != sw_v2_result and sw_v1_result != "NA":
#             error_message = f"Mismatch: v1={sw_v1_result} vs v2={sw_v2_result}"
...
#     with open("test_summary.txt", "w") as f:
#         f.write(f"Test Summary - SW Version: {sw_version_v2}, Build: {job_number}\n")
#         f.write(f"Total: {summary['total']}, Passed: {summary['passed']}, Failed: {summary['failed']}, NA: {summary['na']}, Mismatches: {summary['mismatch']}\n")
#         f.write(f"Selected: {selected}, Deselected: {deselected}, Pass Rate: {(summary['passed'] / summary['total']) * 100:.2f}%\n")
...
def load_latest_results_for_tests(log_csv_path):
    """
    Returns a dictionary with test_case_name as key and its latest known v1 result
    from the most recent job in build_results_log.csv.
    """
    ...
    # Read and sort log file by timestamp (descending)
    df = pd.read_csv(log_csv_path)
    df = df.sort_values(by=["Date time"], ascending=False)

    # Find the column name for SW v2 result dynamically
    sw_v2_col = [col for col in df.columns if col.startswith("SW v2")][0]

    for _, row in df.iterrows():
        test_name = row["Test case name"].strip()
        sw_result = row.get(sw_v2_col, "NA").strip().upper()
        if test_name not in seen:
            latest_results[test_name] = sw_result
            seen.add(test_name)
...
if __name__ == "__main__":
    ...
    sw_version_v1 = "3.220.1.29"  # manually or dynamically managed
    job_number = os.getenv("BUILD_NUMBER", "104")
    timestamp = datetime.now().strftime("%d:%m:%Y %I:%M:%S %p")

    previous_results = load_latest_results_for_tests(output_csv)
    ...
    for test in test_data.get("tests", []):
        test_name = test.get("nodeid", "unknown_test").strip()
        sw_v2_result = test.get("outcome", "UNKNOWN").upper()
        sw_v1_result = previous_results.get(test_name, "NA")
        error_message = ""
